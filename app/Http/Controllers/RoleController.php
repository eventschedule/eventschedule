...
class RoleController extends Controller
{
    ...
    public function viewGuest(Request $request, $subdomain, $slug = '', $id = null, $date = null)
    {
        ...
        if ($event && ! request()->graphic) {
            ...
            $events = Event::with(['roles', 'parts', 'approvedVideos', 'approvedPhotos', 'approvedComments.user', 'activePolls' => fn ($q) => $q->withCount('votes')])->withCount(['approvedVideos', 'approvedComments', 'approvedPhotos', 'activePolls'])
        ...
        if (request()->graphic) {
            if ($role->isCurator()) {
                $pastEvents = Event::with(['roles', 'parts', 'approvedVideos', 'approvedPhotos', 'approvedComments.user', 'activePolls' => fn ($q) => $q->withCount('votes')])->withCount(['approvedVideos', 'approvedComments', 'approvedPhotos', 'activePolls'])
        ...
        if ($embed) {
            ...
            $event->loadMissing(['approvedVideos.user', 'approvedComments.user', 'approvedPhotos.user', 'activePolls' => fn ($q) => $q->withCount('votes')]);
    ...
    public function calendarEvents(Request $request, $subdomain): JsonResponse
    {
        ...
        if ($role->isCurator()) {
            $events = Event::with(['roles', 'parts', 'tickets', 'approvedVideos', 'approvedPhotos', 'approvedComments.user', 'activePolls' => fn ($q) => $q->withCount('votes')])->withCount(['approvedVideos', 'approvedComments', 'approvedPhotos', 'activePolls'])
                ->where(function ($query) use ($startOfMonthUtc) {
                    $query->where('starts_at', '>=', $startOfMonthUtc)
                        ->orWhereNotNull('days_of_week');
                })
        ...
        if ($role->isCurator()) {
            $pastEvents = Event::with(['roles', 'parts', 'tickets', 'approvedVideos', 'approvedPhotos', 'approvedComments.user', 'activePolls' => fn ($q) => $q->withCount('votes')])->withCount(['approvedVideos', 'approvedComments', 'approvedPhotos', 'activePolls'])
                ->where('starts_at', '<', Carbon::now('UTC'))
                ->whereNull('days_of_week')
                ->whereIn('id', function ($query) use ($role) {
                    $query->select('event_id')
