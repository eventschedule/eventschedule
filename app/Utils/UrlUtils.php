...
class UrlUtils
{
    ...
    private const PLATFORM_SVG_PATHS = [
        ...
        'telegram' => 'M9.78 18.65L10.06 14.42L17.74 7.5C18.08 7.19 17.67 7.04 17.23 7.31L7.74 13.3L3.64 12C2.76 11.75 2.75 11.14 3.84 10.7L19.81 4.54C20.54 4.21 21.24 4.72 20.96 5.84L18.24 18.65C18.05 19.56 17.5 19.78 16.74 19.36L12.6 16.3L10.61 18.23C10.38 18.46 10.19 18.65 9.78 18.65Z',
        'threads' => 'M12.186 24h-.007c-3.581-.024-6.334-1.205-8.184-3.509C2.35 18.44 1.5 15.586 1.472 12.01v-.017c.03-3.579.879-6.43 2.525-8.482C5.845 1.205 8.6.024 12.18 0h.014c2.746.02 5.043.725 6.826 2.098 1.677 1.29 2.858 3.13 3.509 5.467l-2.04.569c-1.104-3.96-3.898-5.984-8.304-6.015-2.91.022-5.11.936-6.54 2.717C4.307 6.504 3.616 8.914 3.59 12c.025 3.086.718 5.496 2.057 7.164 1.432 1.781 3.632 2.695 6.54 2.717 2.623-.02 4.358-.631 5.8-2.045 1.647-1.613 1.618-3.593 1.09-4.798-.31-.71-.873-1.3-1.634-1.75-.192 1.352-.622 2.446-1.284 3.272-.886 1.102-2.14 1.704-3.73 1.79-1.202.065-2.361-.218-3.259-.801-1.063-.689-1.685-1.74-1.752-2.96-.065-1.187.408-2.285 1.33-3.09.88-.77 2.132-1.216 3.527-1.258 1.07-.032 2.049.085 2.928.343-.047-.672-.212-1.227-.493-1.66-.394-.607-1.065-.93-1.94-.934h-.036c-.656 0-1.658.175-2.316.945l-1.514-1.3c.982-1.145 2.345-1.75 3.94-1.75h.047c2.514.022 4.09 1.61 4.305 4.317.253.087.49.18.715.28 1.222.545 2.187 1.39 2.787 2.452.742 1.312.952 3.06.3 4.776C20.28 21.14 17.403 23.97 12.186 24zM11.093 15.9c.039.705.584 1.263 1.578 1.263h.089c1.107-.06 1.88-.605 2.36-1.665-.334-.144-.72-.265-1.155-.36-.35-.077-.747-.118-1.178-.118h-.022c-1.053.033-1.704.516-1.672 1.012v-.132z',
    ...
    public static function detectPlatform(string $url): string
    {
        ...
        if (! $host) {
            // Cleaned URL without protocol - extract domain from path-like string
            $host = explode('/', trim($url))[0];
        }

        $host = strtolower(preg_replace('/^www\./', '', $host));

        foreach (self::PLATFORM_DOMAINS as $domain => $platform) {
    ...
    private const PLATFORM_BRAND_NAMES = [
        'facebook' => 'Facebook',
        'instagram' => 'Instagram',
        'youtube' => 'YouTube',
        'whatsapp' => 'WhatsApp',
    ...
    public static function getUniquePlatforms(): array
    {
        return array_values(array_unique(
            array_filter(array_values(self::PLATFORM_DOMAINS), fn ($p) => $p !== 'website')
        ));
    }
    ...
    public static function getSocialSvgPath(string $platform): string
    {
        return self::PLATFORM_SVG_PATHS[$platform] ?? self::PLATFORM_SVG_PATHS['website'];
    }
