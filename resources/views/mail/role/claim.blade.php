    ...
    <div style="background-color: #f9f9f9; padding: 20px; border-radius: 0 0 8px 8px;">
        ...
        <div style="background-color: white; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <div style="text-align: center;">
                @if ($role->getClaimUrl())
                <a href="{{ $role->getClaimUrl() }}" style="display: inline-block; background-color: #2563eb; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: 600;">
                    {{ __('messages.view_schedule') }}
                </a>
                @else
                <a href="{{ route('sign_up', ['email' => base64_encode($role->email)]) }}" style="display: inline-block; background-color: #2563eb; color: white; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: 600;">
                    {{ __('messages.sign_up') }}
                </a>
                @endif
            </div>
        ...
        <p style="font-size: 12px; color: #999; margin-top: 30px; border-top: 1px solid #ddd; padding-top: 20px;">
            {!! __('messages.claim_email_line2', ['click_here' => '<a href="' . route('role.show_unsubscribe', ['email' => base64_encode($role->email)]) . '" style="color: #4E81FA;">' . __('messages.click_here') . '</a>']) !!}
        </p>
