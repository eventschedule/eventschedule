...
<tr>
    <td style="{{ $template === 'bold' ? 'background-color: #16213e;' : ($template === 'classic' ? 'background-color: #faf9f6;' : '') }} padding: {{ $template === 'compact' ? '10px 30px' : '20px 30px' }};">
                            ...
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                                <tr>
                                    ...
                                    <td style="font-size: {{ $template === 'compact' ? '14px' : '15px' }}; font-family: '{{ $style['fontFamily'] }}', sans-serif; vertical-align: top;">
                                        @if ($template === 'minimal')
                                        <a href="{{ $event->getGuestUrl($role?->subdomain, null, true) }}" style="color: {{ $style['accentColor'] }}; text-decoration: underline; font-weight: bold;">{{ $event->name }}</a>
                                        @else
                                        <a href="{{ $event->getGuestUrl($role?->subdomain, null, true) }}" style="color: {{ $template === 'bold' ? '#ffffff' : $style['textColor'] }}; text-decoration: none; font-weight: bold;">{{ $event->name }}</a>
                                        @endif
                                        @php
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 12px; border-bottom: 1px solid #eee;">
                    <tr>
                        <td style="padding: 12px 0; vertical-align: top; font-family: '{{ $style['fontFamily'] }}', sans-serif;">
                            ...
                            @endif
                            <a href="{{ $event->getGuestUrl($role?->subdomain, null, true) }}" style="color: {{ $style['accentColor'] }}; text-decoration: underline; font-size: 13px; font-family: '{{ $style['fontFamily'] }}', sans-serif;">{{ __('messages.view_event') ?: 'View Event' }} {!! $arrow ?? '&rarr;' !!}</a>
                        </td>
                    </tr>
                </table>
                @endforeach
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 8px; border: 1px solid #eee;">
                    <tr>
                        <td style="padding: 8px 12px; vertical-align: top; font-family: '{{ $style['fontFamily'] }}', sans-serif;">
                            ...
                            @endif
                            <a href="{{ $event->getGuestUrl($role?->subdomain, null, true) }}" style="display: inline-block; padding: 4px 10px; background-color: {{ $style['accentColor'] }}; color: #ffffff; text-decoration: none; font-size: 11px; font-weight: bold; {{ $style['buttonRadius'] === 'rounded' ? 'border-radius: 3px;' : '' }} font-family: '{{ $style['fontFamily'] }}', sans-serif;">{{ __('messages.view_event') ?: 'View Event' }}</a>
                        </td>
                    </tr>
                </table>
                @endforeach
            @else
                ...
                @php
                    ...
                    if ($template === 'classic') {
                        ...
                        $cardExtra = ($startBorder ?? 'border-left') . ': 4px solid ' . $style['accentColor'] . ';';
                        $titleColor = '#ffffff';
                    }
                @endphp
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 16px; border: {{ $cardBorder }}; {{ $style['buttonRadius'] === 'rounded' ? 'border-radius: 8px;' : '' }} {{ $cardBg }} {{ $cardExtra }}">
                    <tr>
                        ...
                        <td style="padding: 14px 16px; vertical-align: top; font-family: '{{ $style['fontFamily'] }}', sans-serif;">
                            ...
                            @endif
                            <a href="{{ $event->getGuestUrl($role?->subdomain, null, true) }}" style="display: inline-block; padding: 8px 16px; {{ $btnStyle }} text-decoration: none; font-size: 13px; font-weight: bold; {{ $style['buttonRadius'] === 'rounded' ? 'border-radius: 4px;' : '' }}">{{ __('messages.view_event') ?: 'View Event' }}</a>
                        </td>
                    </tr>
                </table>
                @endforeach
