...
<head>
    ...
    <title>{{ __('messages.waitlist_tickets_available') }}</title>
</head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background-color: #4E81FA; color: white; padding: 30px 20px; text-align: center; border-radius: 8px 8px 0 0;">
        <h1 style="margin: 0; font-size: 24px; font-weight: 600;">{{ __('messages.waitlist_tickets_available') }}</h1>
    </div>

    <div style="background-color: #f9f9f9; padding: 20px; border-radius: 0 0 8px 8px;">
        <p style="font-size: 16px; margin-top: 0;">{{ __('messages.hello') }}, {{ $entry->name }}!</p>

        <p style="font-size: 16px;">{{ __('messages.waitlist_notification_body') }}</p>
        ...
        <div style="background-color: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #4E81FA;">
            ...
            <p style="margin: 0; font-size: 14px; color: #666;">
                {{ $event->getStartDateTime($entry->event_date, true)->format('F j, Y') }}
        ...
        <div style="text-align: center; margin: 30px 0;">
            <a href="{{ $eventUrl }}"
               style="display: inline-block; background-color: #4E81FA; color: white; padding: 15px 30px; text-decoration: none; border-radius: 5px; font-weight: bold; font-size: 16px;">
                {{ __('messages.waitlist_notification_cta') }}
            </a>
        </div>
