<x-app-admin-layout>
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Plan</h1>
        ...
        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            <h3 class="text-sm font-medium text-blue-800 dark:text-blue-300 mb-3">Current Subscription Status</h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                    <span class="text-gray-600 dark:text-gray-400">Stripe Customer:</span>
                    <span class="ml-2 font-medium text-gray-900 dark:text-white">{{ $role->stripe_id ?: 'None' }}</span>
                </div>
                ...
                <div>
                    <span class="text-gray-600 dark:text-gray-400">Active Subscription:</span>
        ...
        <form method="POST" action="{{ route('admin.plans.update', ['role' => $role->encodeId()]) }}" class="bg-white dark:bg-gray-800 rounded-lg shadow">
            ...
            <div class="p-6 space-y-6">
                {{-- Plan Type --}}
                <div>
                    <label for="plan_type" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Plan Type</label>
                    <select name="plan_type" id="plan_type" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="free" {{ ($role->plan_type ?? 'free') === 'free' ? 'selected' : '' }}>Free</option>
                        <option value="pro" {{ $role->plan_type === 'pro' ? 'selected' : '' }}>Pro</option>
                        <option value="enterprise" {{ $role->plan_type === 'enterprise' ? 'selected' : '' }}>Enterprise</option>
                    </select>
                    @error('plan_type')
                ...
                {{-- Plan Expires --}}
                <div>
                    <label for="plan_expires" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Plan Expires</label>
