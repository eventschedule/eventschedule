    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('admin.referrals') }}"
                class="px-3 py-1 rounded-full text-sm {{ !$statusFilter ? 'bg-[#4E81FA] text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                {{ __('messages.all') }}
            </a>
            @foreach (['pending', 'subscribed', 'qualified', 'credited', 'expired'] as $status)
            <a href="{{ route('admin.referrals', ['status' => $status]) }}"
                class="px-3 py-1 rounded-full text-sm {{ $statusFilter === $status ? 'bg-[#4E81FA] text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                {{ __('messages.' . $status) }}
            </a>
            @endforeach
        </div>
