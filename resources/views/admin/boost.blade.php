    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                ...
                <p class="text-xs text-gray-400 dark:text-gray-500">{{ number_format($totalCampaignsInPeriod) }} in period</p>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                <p class="text-sm text-gray-500 dark:text-gray-400">Markup Revenue</p>
                <p class="text-2xl font-bold text-gray-900 dark:text-white">${{ number_format($markupRevenue, 2) }}</p>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                <p class="text-sm text-gray-500 dark:text-gray-400">Total Ad Spend</p>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                <p class="text-sm text-gray-500 dark:text-gray-400">Avg CTR</p>
                <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ number_format($avgCtr, 2) }}%</p>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                <p class="text-sm text-gray-500 dark:text-gray-400">Avg CPC</p>
                <p class="text-2xl font-bold text-gray-900 dark:text-white">${{ number_format($avgCpc, 2) }}</p>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                <p class="text-sm text-gray-500 dark:text-gray-400">Avg CPM</p>
        ...
        <div class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg p-4">
            <div class="flex items-start">
                ...
                <div class="space-y-2 w-full">
                    ...
                    <p class="text-sm text-red-700 dark:text-red-300 font-medium">{{ $stuckPending->count() }} campaign(s) stuck in pending_payment for over 30 minutes</p>
                    ...
                    <p class="text-sm text-red-700 dark:text-red-300 font-medium">{{ $failedCampaigns->count() }} failed campaign(s)</p>
                    <ul class="text-xs text-red-600 dark:text-red-400 list-disc list-inside">
                    ...
                    <p class="text-sm text-red-700 dark:text-red-300 font-medium">{{ $disapprovedCampaigns->count() }} campaign(s) with disapproved ads</p>
                    <ul class="text-xs text-red-600 dark:text-red-400 list-disc list-inside">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {{-- Top Boosters --}}
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Top Boosters</h3>
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="text-left text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                                ...
                                <th class="pb-2 font-medium text-end">Campaigns</th>
                                <th class="pb-2 font-medium text-end">Budget</th>
                                <th class="pb-2 font-medium text-end">Spend</th>
                                <th class="pb-2 font-medium text-end">Clicks</th>
                                <th class="pb-2 font-medium text-end">Limit</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            @foreach ($topBoosters as $booster)
                            @php
                                $boosterRole = \App\Models\Role::find($booster->role_id);
                            @endphp
                            <tr>
                                <td class="py-2 text-gray-900 dark:text-white">{{ $topBoosterRoles[$booster->role_id] ?? 'N/A' }}</td>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Grant Boost Credit</h3>

            <form action="{{ route('admin.boost.grant_credit') }}" method="POST" class="flex flex-wrap gap-4 items-end mb-6">
                @csrf
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Schedule Subdomain</label>
            ...
            <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Schedules with Credit</h4>
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="text-left text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                            <th class="pb-2 font-medium">Subdomain</th>
                            <th class="pb-2 font-medium text-end">Balance</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            @else
            <p class="text-sm text-gray-500 dark:text-gray-400">No schedules with boost credit.</p>
            @endif
        </div>

        {{-- Set Spending Limit Section --}}
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Set Spending Limit</h3>
            ...
            <form action="{{ route('admin.boost.set_limit') }}" method="POST" class="flex flex-wrap gap-4 items-end mb-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Max Budget per Campaign ($)</label>
            ...
            <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Schedules with Custom Limits</h4>
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="text-left text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                            <th class="pb-2 font-medium">Subdomain</th>
                            <th class="pb-2 font-medium text-end">Max Budget</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            @else
            <p class="text-sm text-gray-500 dark:text-gray-400">No schedules with custom spending limits (all using default ${{ number_format(config('services.meta.boost_default_limit', 10), 0) }}).</p>
            @endif
        </div>
                ...
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="text-left text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                            ...
                            <th class="pb-2 font-medium text-end">Budget</th>
                            <th class="pb-2 font-medium text-end">Spend</th>
                            <th class="pb-2 font-medium text-end">Impr.</th>
                            <th class="pb-2 font-medium text-end">Clicks</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        ...
                        <tr>
                            ...
                            <td class="py-2">
                                <a href="{{ route('boost.show', ['hash' => $campaign->hashedId()]) }}" class="text-blue-600 dark:text-blue-400 hover:underline text-xs">View</a>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="11" class="py-4 text-center text-gray-500 dark:text-gray-400">No campaigns found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Recent Billing Records</h3>
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    <thead>
                        <tr class="text-left text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                            ...
                            <th class="pb-2 font-medium text-end">Amount</th>
                            <th class="pb-2 font-medium text-end">Markup</th>
                            <th class="pb-2 font-medium">Status</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        ...
                        @empty
                        <tr>
                            <td colspan="7" class="py-4 text-center text-gray-500 dark:text-gray-400">No billing records.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
