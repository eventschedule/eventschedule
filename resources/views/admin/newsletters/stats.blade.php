<x-app-admin-layout>
    ...
    <div class="space-y-4">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{ __('messages.sent') }}: {{ $newsletter->sent_at ? $newsletter->sent_at->format(get_use_24_hour_time(null) ? 'M j, Y H:i' : 'M j, Y g:i A') : '-' }}
                </p>
            </div>
        ...
        <div class="ap-card rounded-xl shadow p-6 mb-8">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-3 text-sm text-gray-600 dark:text-gray-400">
                            {{ $recipient->opened_at ? \Carbon\Carbon::parse($recipient->opened_at)->format(get_use_24_hour_time(null) ? 'M j, H:i' : 'M j, g:i A') : '-' }}
                            @if ($recipient->open_count > 1)
                            <span class="text-xs text-gray-400">({{ $recipient->open_count }}x)</span>
                            @endif
                        </td>
                        <td class="px-6 py-3 text-sm text-gray-600 dark:text-gray-400">
                            {{ $recipient->clicked_at ? \Carbon\Carbon::parse($recipient->clicked_at)->format(get_use_24_hour_time(null) ? 'M j, H:i' : 'M j, g:i A') : '-' }}
