...
&lt;iframe src="{{ route('role.view_guest', ['subdomain' => 'your-schedule-name']) }}?embed=true&#38;layout=list"
    ...
    <section id="social" class="doc-section">
        ...
        <p class="text-gray-600 dark:text-gray-300 mb-4">Each event has its own URL. Its preview card uses the event name as the title. The image is the event flyer. Without one it is a profile image the event page itself shows: the first performer's that has one, else the venue's, else that of the schedule whose page it is, whichever schedule created the event. A deleted schedule, or one that declined the event, is passed over, and if that leaves none of those photos the card has no image. The description is the event's short description, joined to its full description when the short one is under 50 characters, or else the event name, followed by the date and the venue while there is room. A recurring event's own link stands for every date, so it leaves the date out. In search results and browser tabs, the page title adds the venue or town to the event name while they fit, and the date too on a one-off event.</p>
    ...
    <section id="calendar-feeds" class="doc-section">
        ...
        <div class="doc-fields">
            ...
            <div class="doc-field">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Each event page has an <strong class="text-gray-900 dark:text-white">Add to Calendar</strong> button offering Google Calendar, Apple Calendar (which downloads an .ics file) and Microsoft Outlook. Each of those copies one event across once and never updates. The rows below them are the ones that keep someone up to date: <strong class="text-gray-900 dark:text-white">Subscribe to all events from</strong> your schedule is the iCal feed above, and once you switch on the <a href="{{ route('marketing.docs.creating_schedules') }}#settings-advanced" class="doc-link">&ldquo;Notify me&rdquo; card</a>, <strong class="text-gray-900 dark:text-white">Tell me when tickets go on sale</strong> puts an email address on the <a href="{{ route('marketing.docs.tickets') }}#interest-list" class="doc-link">interest list</a> of a public event that has not happened yet.</p>
            </div>
        </div>
