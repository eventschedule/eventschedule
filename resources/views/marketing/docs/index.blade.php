@php
    ...
    $glossary = [
        ...
        ['term' => 'Interest list', 'def' => 'The email addresses left on one event with "Tell me when tickets go on sale", or "Tell me if anything changes" once it is selling, on the “Notify me” card that a schedule switches on for the events it creates. No account and no name. They hear about that date only: when tickets go on sale, a reminder before it starts, a notice if it is cancelled, and any notice you choose to send if the date or venue changes. It is not a subscription to your schedule.'],
