    ...
    <button @click="toggleDropdown" type="button" tabindex="0"
        ...
        <template v-if="selectedEvent">
            ...
            <span class="flex-1 min-w-0">
                ...
                <span v-if="selectedEvent.starts_at" class="block truncate text-gray-500 dark:text-[#9ca3af] text-xs">@{{ selectedEvent.starts_at }}</span>
            </span>
        </template>
        <span v-else class="flex-1 text-gray-500 dark:text-[#9ca3af] text-sm">{{ $placeholder ?? __('messages.select_event') }}</span>
