                ...
                @else
                    <div class="flex items-center justify-between w-full">
                        ...
                        {{-- The link renders whether or not we have a count, and the count is the
                             ...
                             the star count is refreshed on nexus only, gating the whole anchor on
                             $githubStars removed the "star us" affordance from the one install type
                             that shows it at all.

                             Both glyphs are decorative, so without aria-label the link would
    ...
    <x-modal name="about-app" maxWidth="md" :ariaLabel="__('messages.about')">
        <div class="relative">
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 divide-y divide-gray-200 dark:divide-gray-700">
                ...
                {{-- Source. The star count is a STORED settings value, not a live fetch or a
                     cache entry: GitHubUtils::getStars() reads whatever app:check-github-stars
                     last wrote, and that command no-ops off the nexus - so on a selfhost install
                ...
                @if ($aboutHosted)
                <a href="{{ policy_url('terms') }}" target="_blank" rel="noopener noreferrer" class="{{ $aboutRow }}">
                    <svg class="{{ $aboutIcon }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
                ...
                <a href="{{ policy_url('privacy') }}" target="_blank" rel="noopener noreferrer" class="{{ $aboutRow }}">
                    <svg class="{{ $aboutIcon }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
