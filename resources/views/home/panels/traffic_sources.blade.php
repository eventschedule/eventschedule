<div class="ap-card rounded-xl h-full flex flex-col overflow-hidden">
    <div class="dashboard-panel-header px-5 py-4">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('messages.panel_traffic_sources') }} ({{ $panelSettings['traffic_sources']['period'] ?? 30 }}d)</h2>
        </div>
    </div>
    <div class="divide-y divide-gray-100 dark:divide-white/[0.06] flex-1">
        @forelse($trafficSources as $source)
        <div class="flex items-center justify-between px-5 py-3">
            <span class="text-sm text-gray-700 dark:text-gray-300 truncate me-3">{{ $source['domain'] }}</span>
            <span class="text-sm font-semibold text-gray-900 dark:text-white flex-shrink-0">{{ number_format($source['view_count']) }} {{ mb_strtolower(__('messages.views')) }}</span>
        </div>
        @empty
