                    ...
                    @if ($each->isClaimed() && (config('app.hosted') || config('app.is_testing')) && ! is_demo_mode())
                      ...
                      @else
                        <button type="button"
                          ...
                          data-account-note="{{ $each->willCreateAccountOnConfirm() ? '1' : '' }}"
          ...
          <div class="p-5 relative z-10">
            ...
            <div class="flex flex-col gap-2">
              ...
              <div class="flex flex-col gap-4 mt-2">
                ...
                <div class="flex flex-row gap-2 items-center relative duration-300 text-gray-700 dark:text-gray-300 fill-gray-700 dark:fill-gray-300 min-w-0 {{ $role->isRtl() ? 'rtl' : '' }}">
                  <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="flex-shrink-0">
              ...
              <div class="relative mt-4">
                ...
                <div id="calendar-card-dropdown" class="pop-up-menu gp-dropdown hidden absolute top-full {{ $role->isRtl() ? 'end-0 origin-top-right' : 'start-0 origin-top-left' }} z-50 mt-2 w-56 rounded-xl bg-white shadow-lg focus:outline-none" role="menu" aria-orientation="vertical" tabindex="-1">
                  <div class="py-1 stop-propagation" role="none">
                    <a href="{{ $event->getGoogleCalendarUrl($date) }}" target="_blank" rel="noopener noreferrer" class="group flex items-center px-4 py-3 text-sm text-gray-700 dark:text-gray-200 rounded-lg transition-all duration-200 gp-dropdown-item" role="menuitem" tabindex="-1">
                    ...
                    <a href="{{ $event->getAppleCalendarUrl($date, $role->subdomain) }}" class="group flex items-center px-4 py-3 text-sm text-gray-700 dark:text-gray-200 rounded-lg transition-all duration-200 gp-dropdown-item" role="menuitem" tabindex="-1">
                      <svg class="me-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                        <path d="M18.71,19.5C17.88,20.74 17,21.95 15.66,21.97C14.32,22 13.89,21.18 12.37,21.18C10.84,21.18 10.37,21.95 9.1,22C7.79,22.05 6.8,20.68 5.96,19.47C4.25,17 2.94,12.45 4.7,9.39C5.57,7.87 7.13,6.91 8.82,6.88C10.1,6.86 11.32,7.75 12.11,7.75C12.89,7.75 14.37,6.68 15.92,6.84C16.57,6.87 18.39,7.1 19.56,8.82C19.47,8.88 17.39,10.1 17.41,12.63C17.44,15.65 20.06,16.66 20.09,16.67C20.06,16.74 19.67,18.11 18.71,19.5M13,3.5C13.73,2.67 14.94,2.04 15.94,2C16.07,3.17 15.6,4.35 14.9,5.19C14.21,6.04 13.07,6.7 11.95,6.61C11.8,5.46 12.36,4.26 13,3.5Z" />
                      </svg>
                      Apple Calendar
                    </a>
                    <a href="{{ $event->getMicrosoftCalendarUrl($date) }}" target="_blank" rel="noopener noreferrer" class="group flex items-center px-4 py-3 text-sm text-gray-700 dark:text-gray-200 rounded-lg transition-all duration-200 gp-dropdown-item" role="menuitem" tabindex="-1">
                      <svg class="me-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        ...
        @if (!empty($showCalendarPopup))
            ...
            <div id="calendar-pop-up-menu" class="pop-up-menu gp-dropdown hidden absolute top-full end-0 z-50 mt-2 w-72 {{ $role->isRtl() ? 'origin-top-left' : 'origin-top-right' }} rounded-xl bg-white shadow-lg focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                <div class="py-1 stop-propagation" role="none">
                    ...
                    <a href="{{ $event->getAppleCalendarUrl($date, $role->subdomain) }}" class="group flex items-center px-4 py-3 text-sm text-gray-700 dark:text-gray-200 rounded-lg transition-all duration-200 gp-dropdown-item" role="menuitem" tabindex="-1" id="menu-item-1">
                        <svg class="me-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                        <path d="M18.71,19.5C17.88,20.74 17,21.95 15.66,21.97C14.32,22 13.89,21.18 12.37,21.18C10.84,21.18 10.37,21.95 9.1,22C7.79,22.05 6.8,20.68 5.96,19.47C4.25,17 2.94,12.45 4.7,9.39C5.57,7.87 7.13,6.91 8.82,6.88C10.1,6.86 11.32,7.75 12.11,7.75C12.89,7.75 14.37,6.68 15.92,6.84C16.57,6.87 18.39,7.1 19.56,8.82C19.47,8.88 17.39,10.1 17.41,12.63C17.44,15.65 20.06,16.66 20.09,16.67C20.06,16.74 19.67,18.11 18.71,19.5M13,3.5C13.73,2.67 14.94,2.04 15.94,2C16.07,3.17 15.6,4.35 14.9,5.19C14.21,6.04 13.07,6.7 11.95,6.61C11.8,5.46 12.36,4.26 13,3.5Z" />
                        </svg>
                        Apple Calendar
                    </a>
                    <a href="{{ $event->getMicrosoftCalendarUrl($date) }}" target="_blank" rel="noopener noreferrer" class="group flex items-center px-4 py-3 text-sm text-gray-700 dark:text-gray-200 rounded-lg transition-all duration-200 gp-dropdown-item" role="menuitem" tabindex="-1" id="menu-item-2">
                        <svg class="me-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                        <path d="M2,3H11V12H2V3M11,22H2V13H11V22M21,3V12H12V3H21M21,22H12V13H21V22Z" />
                        </svg>
                        Microsoft Outlook
        ...
        <div id="calendar-mobile-sheet" class="hidden fixed inset-0 z-50 sm:hidden">
          ...
          <div class="gp-bottom-sheet fixed inset-x-0 bottom-0 rounded-t-2xl shadow-xl">
            <div class="flex justify-center py-3">
              <div class="gp-bottom-sheet-handle w-12 h-1.5 rounded-full"></div>
            </div>
            <div class="px-6 pb-6 space-y-1 {{ $role->isRtl() ? 'rtl' : '' }}">
              <a href="{{ $event->getGoogleCalendarUrl($date) }}" target="_blank" rel="noopener noreferrer" class="gp-bottom-sheet-item flex items-center gap-3 px-4 py-3 rounded-xl text-gray-700 transition-all duration-200">
                <svg class="h-5 w-5 text-gray-400" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M21.35,11.1H12.18V13.83H18.69C18.36,17.64 15.19,19.27 12.19,19.27C8.36,19.27 5,16.25 5,12C5,7.9 8.2,4.73 12.2,4.73C15.29,4.73 17.1,6.7 17.1,6.7L19,4.72C19,4.72 16.56,2 12.1,2C6.42,2 2.03,6.8 2.03,12C2.03,17.05 6.16,22 12.25,22C17.6,22 21.5,18.33 21.5,12.91C21.5,11.76 21.35,11.1 21.35,11.1V11.1Z" />
                </svg>
                Google Calendar
              </a>
              <a href="{{ $event->getAppleCalendarUrl($date, $role->subdomain) }}" class="gp-bottom-sheet-item flex items-center gap-3 px-4 py-3 rounded-xl text-gray-700 transition-all duration-200">
                <svg class="h-5 w-5 text-gray-400" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M18.71,19.5C17.88,20.74 17,21.95 15.66,21.97C14.32,22 13.89,21.18 12.37,21.18C10.84,21.18 10.37,21.95 9.1,22C7.79,22.05 6.8,20.68 5.96,19.47C4.25,17 2.94,12.45 4.7,9.39C5.57,7.87 7.13,6.91 8.82,6.88C10.1,6.86 11.32,7.75 12.11,7.75C12.89,7.75 14.37,6.68 15.92,6.84C16.57,6.87 18.39,7.1 19.56,8.82C19.47,8.88 17.39,10.1 17.41,12.63C17.44,15.65 20.06,16.66 20.09,16.67C20.06,16.74 19.67,18.11 18.71,19.5M13,3.5C13.73,2.67 14.94,2.04 15.94,2C16.07,3.17 15.6,4.35 14.9,5.19C14.21,6.04 13.07,6.7 11.95,6.61C11.8,5.46 12.36,4.26 13,3.5Z" />
                </svg>
                Apple Calendar
              </a>
              <a href="{{ $event->getMicrosoftCalendarUrl($date) }}" target="_blank" rel="noopener noreferrer" class="gp-bottom-sheet-item flex items-center gap-3 px-4 py-3 rounded-xl text-gray-700 transition-all duration-200">
                <svg class="h-5 w-5 text-gray-400" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M2,3H11V12H2V3M11,22H2V13H11V22M21,3V12H12V3H21M21,22H12V13H21V22Z" />
                </svg>
                Microsoft Outlook
              ...
              <div class="relative mb-6 last:mb-0">
                ...
                <div class="flex flex-col bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
                  ...
                  <div class="mt-2 flex flex-wrap gap-3" x-data="{ showVideo: false, showComment: false, showPhoto: false, dragging: false, photoPreview: null }">
                    ...
                    @endif
                    {{-- On its own line: the parent is a flex row of buttons, so the widget
                         would otherwise be laid out as one of them. Outside the per-type @if blocks
                         so it renders whenever any fan-content type is enabled. --}}
        ...
        <div id="gp-fan-content" class="bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm sm:rounded-2xl p-6 sm:p-8 {{ $role->isRtl() ? 'rtl' : '' }}">
          ...
          <div class="space-y-4 mb-6" x-data="pollsComponent()">
              ...
              <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                  ...
                  @auth
                      @php
                          $pollEventDate = ($event->days_of_week && $date) ? $date : '';
                          $userVote = $poll->getUserVote(auth()->id(), $pollEventDate);
                      @endphp
                      ...
                      @else
                          ...
                          @php
                              $results = $poll->getResults($pollEventDate);
                              $totalVotes = $pollEventDate !== '' ? $poll->votes()->where('event_date', $pollEventDate)->count() : $poll->votes()->count();
                      ...
                      @endif
                      @if ($poll->is_active && $poll->allow_user_options && count($poll->options) < 10)
                          @php $pollHash = $pollHash ?? \App\Utils\UrlUtils::encodeId($poll->id); @endphp
                          <div class="mt-3 flex gap-2">
                              <input type="text" x-model="suggestInput['{{ $pollHash }}']" maxlength="200"
                                     placeholder="{{ __('messages.suggest_option') }}"
                                     @keydown.enter="suggestOption('{{ $pollHash }}')"
                          ...
                          <p x-show="suggestMessage['{{ $pollHash }}']" x-text="suggestMessage['{{ $pollHash }}']" class="text-xs text-gray-500 dark:text-gray-400 mt-1" x-cloak></p>
                      @endif
                  @else
                      @if ($poll->is_active)
  ...
  <script {!! nonce_attr() !!}>
    function pollsComponent() {
      return {
        ...
        async vote(pollHash, optionIndex, clickEvent, buttonEl) {
          ...
          try {
            const resp = await fetch('{{ route('event.vote_poll', ['subdomain' => $role->subdomain, 'event_hash' => \App\Utils\UrlUtils::encodeId($event->id), 'poll_hash' => 'POLL_HASH']) }}'.replace('POLL_HASH', pollHash), {
              ...
              body: JSON.stringify({ option_index: optionIndex, event_date: '{{ ($event->days_of_week && $date) ? $date : '' }}' }),
            });
            if (resp.status === 401) {
        ...
        async suggestOption(pollHash) {
          var label = (this.suggestInput[pollHash] || '').trim();
          if (!label || this.suggestSubmitting[pollHash]) return;
          this.suggestSubmitting[pollHash] = true;
          this.suggestMessage[pollHash] = '';
          try {
            ...
            if (data.success) {
              this.suggestInput[pollHash] = '';
              this.suggestMessage[pollHash] = data.message;
              if (data.options) {
                // Option was added directly - reload to show it
                window.location.reload();
              }
            } else {
              this.suggestMessage[pollHash] = data.error || '{{ __("messages.an_error_occurred") }}';
            }
          } catch (e) {
            this.suggestMessage[pollHash] = '{{ __("messages.an_error_occurred") }}';
          } finally {
            this.suggestSubmitting[pollHash] = false;
          }
        }
      }
    }
