<x-app-guest-layout :role="$role" :event="$event" :date="$date" :fonts="$fonts">
  ...
  @endif
    ...
    <article
      class="container mx-auto flex flex-col sm:grid px-5 pb-[80px] lg:gap-[48px] gap-[8px] lg:grid-cols-[minmax(0px,_auto)_minmax(0px,_344px)]"
    >
      <div class="flex flex-col gap-10">
        @if (request()->get('tickets') === 'true' && $event->isPro())
        <div class="flex flex-col xl:flex-row gap-10 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm px-5 py-6 sm:p-8 mb-4">
          <div class="flex-1">
        ...
        @if ($translation ? $translation->description_translated : $event->translatedDescription())
          <div class="mb-6 flex flex-col gap-4 {{ $role->isRtl() || ($translation && $translation->role->isRtl()) ? 'rtl' : '' }}">
          ...
          <div class="border-t border-gray-200 dark:border-gray-700 pt-6 mb-6 {{ $role->isRtl() ? 'rtl' : '' }}">
                  ...
                  <div class="flex flex-col">
                    ...
                    <div class="mt-2 flex gap-3" x-data="{ showVideo: false, showComment: false }">
                      <button @click="showVideo = !showVideo; showComment = false" class="text-xs flex items-center gap-1 hover:underline" style="color: {{ $accentColor }};">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3.5 h-3.5" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="m15.75 10.5 4.72-4.72a.75.75 0 0 1 1.28.53v11.38a.75.75 0 0 1-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25h-9A2.25 2.25 0 0 0 2.25 7.5v9a2.25 2.25 0 0 0 2.25 2.25Z" /></svg>
                      ...
                      <button @click="showComment = !showComment; showVideo = false" class="text-xs flex items-center gap-1 hover:underline" style="color: {{ $accentColor }};">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3.5 h-3.5" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 0 1 .865-.501 48.172 48.172 0 0 0 3.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018Z" /></svg>
          ...
          @foreach ($event->members() as $each)
            @if (! $each->isClaimed() && ! $each->getFirstVideoUrl())
              @continue
            @endif
          <div class="border-t border-gray-200 dark:border-gray-700 pt-6 mb-6 flex flex-col gap-4">
            <div
            ...
            >
              <div class="flex gap-3 flex-row items-center">
                ...
                >
                  <h2 class="text-[28px] font-semibold leading-10 text-gray-900 dark:text-gray-100 group-hover:underline" style="font-family: '{{ $each->font_family }}', sans-serif;">
                    {{ $each->name }}
                  </h2>
                ...
                <img
                  src="{{ $each->profile_image_url }}"
                  alt="{{ $each->translatedName() }}"
                  class="rounded-full w-16 h-16 object-cover"
                />
                @endif
                <h2 class="text-[28px] font-semibold leading-10 text-gray-900 dark:text-gray-100" style="font-family: '{{ $each->isClaimed() ? $each->font_family : $otherRole->font_family }}', sans-serif;">
                  {{ $each->translatedName() }}
                </h2>
                @endif
              </div>
            ...
            <div class="text-base text-gray-700 dark:text-gray-300 custom-content {{ $role->isRtl() ? 'rtl' : '' }}">
      ...
      <aside class="flex flex-col gap-6 {{ $role->isRtl() ? 'rtl' : '' }}">
        @if ($event->venue && $event->venue->name)
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div class="h-1" style="background-color: {{ $otherRole->accent_color ?? '#4E81FA' }};"></div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 flex flex-col gap-4 {{ $role->isRtl() ? 'rtl' : '' }}">
          <h2 class="text-gray-900 dark:text-gray-100 text-base leading-snug font-semibold">
            {{ __('messages.add_to_calendar') }}
          </h2>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 flex flex-col gap-6 {{ $role->isRtl() ? 'rtl' : '' }}">
        ...
        {{-- Calendar section --}}
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 sm:p-6 flex flex-col gap-6 {{ $role->isRtl() ? 'rtl' : '' }}">
  ...
  {{-- Sticky mobile CTA --}}
  <div class="fixed bottom-0 inset-x-0 z-40 sm:hidden bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 px-5 py-3 shadow-lg" style="font-family: sans-serif">
    @if ($event->canSellTickets($date) || $event->registration_url)
      @if (request()->get('tickets') !== 'true')
        <a href="{{ $event->registration_url ? $event->registration_url : request()->fullUrlWithQuery(['tickets' => 'true']) }}" {{ $event->registration_url ? 'target="_blank"' : '' }}
          @if ($event->payment_method === 'payment_url' && $event->user && $event->user->paymentUrlMobileOnly() && ! is_mobile())
        ...
        >
          <button type="button"
                class="w-full justify-center rounded-md px-6 py-3 text-lg font-semibold shadow-sm hover:opacity-90"
                style="background-color: {{ $accentColor }}; color: {{ $contrastColor }};">
            {{ $event->registration_url ? __('messages.view_event') : ($event->areTicketsFree() ? __('messages.get_tickets') : __('messages.buy_tickets')) }}
          </button>
        </a>
    ...
    @else
      <button type="button"
          ...
          class="w-full justify-center gap-x-1.5 rounded-md px-6 py-3 text-lg font-semibold shadow-sm hover:opacity-90 flex items-center"
          style="background-color: {{ $accentColor }}; color: {{ $contrastColor }};">
        {{ __('messages.add_to_calendar') }}
        <svg class="-me-1 h-5 w-5" style="color: {{ $contrastColor }};" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
          <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd" />
