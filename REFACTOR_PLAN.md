...
| P4 | not started | | | | |
| P5 | tests-first done (refactor pending) | merged to main @ 9ad67a11 | | 2026-07-11 | EncodedIdRoutingCharacterizationTest: 404-parity triples on event.edit + newsletter.edit. NOTE: the venue_id body-param site 404s on an invalid hash (chained findOrFail - it does NOT fall through to the no-venue path); the work order's nullable-find example was wrong for this site, trust the pinned test. |
| P6 | not started | | | | |
| P7 | tests-first done (split pending) | merged to main @ 9ad67a11 | | 2026-07-11 | RoleGuestSurfaceCharacterizationTest (11 tests): viewGuest minimum set (home/slug/id/date/embed), unknown slug -> 302 to schedule home (NOT 404), unknown subdomain -> 302 app_url() (NOT 404), calendarEvents JSON shape + unclaimed empty shell, follow/unfollow + guest sign-up redirect, unsubscribe page, followers QR PNG, past-events before-param gate. |
| P8 | tests-first partial (split pending) | merged to main @ 9ad67a11 | | 2026-07-11 | resendInvite (stub notify + already-signed-up reject) in EventCurationModerationCharacterizationTest; team add/update/remove already in MiscFeaturesTest; merge flows already in ScheduleFeaturesTest. AI-cluster endpoints untested (need Gemini/OpenAI mocks - route smokes only via RouteLoadTest). |
| P9 | tests-first done (split pending) | merged to main @ 9ad67a11 | | 2026-07-11 | Curate flow (editor accepted / guest unaccepted / duplicate error / uncurate) + photo approve/reject pair in EventCurationModerationCharacterizationTest; guest-import in ScheduleFeaturesTest; fan content + poll vote in EventTest. NOTE: storeMember stub path needs an inviter hydrated from DB (language_code) - actingAs with a non-fresh factory user 500s; that is a fixture artifact, not an app bug. |
| P10 | tests-first done (split pending) | merged to main @ 9ad67a11 | | 2026-07-11 | CheckoutBranchCharacterizationTest: cash branch end-to-end (unpaid sale + ticket.view redirect), payment_url redirect-out (event OWNER's payment_url verbatim), promo-on-paid-sale (payment_amount + discount_amount), volume discount, passBook (garbage-date reject + booking entry in sale_tickets.pass_usages JSON - no bookings table). Stripe/InvoiceNinja branches need live APIs (P15 will characterize up to redirect-out with HTTP fakes if needed). TicketingTest/PassBookingTest remain the backbone. |
| P11 | done | refactor/p11-saveevent-characterization (deleted) | 9ad67a11 | 2026-07-11 | 43 tests / 6 classes in tests/Feature/Characterization/ (venue, members, schedule/recurrence, tickets, curators, lifecycle, custom-fields) + SavesEventsOverHttp driver trait + CreatesScheduleData helpers (createGroup/createCurator/createVenueWithAddress/createRecurringEvent/followRole). Found BUG-001 (claim_venue_ownership verified-at copy undone by Role::updating hook) - pinned, see BUGS_FOUND.md. Notable pinned quirks: non-pass tickets on one-time events store pass_usage_type='total'; new venues attach with is_accepted=null (require_approval default TRUE); the flyer upload field is flyer_image (flyer_image_url rule is vestigial); custom-field whitelisting is two-layer (FormRequest Rule::in on the form path, repo whitelist for synthesized requests). Full suite 220 passed / 3 skipped. |
| P12 | not started | | | | |
| P13 (opt) | not started | | | | |
| P14 | tests-first done (decomposition pending) | merged to main @ 9ad67a11 | | 2026-07-11 | ViewGuestDeepCharacterizationTest (password gate incl. wrong-password `password_error` flash, private-event member/guest split, ticket embed, curator cross-schedule listing + is_accepted gate) + RoleUpdateCharacterizationTest (full roles-row column map, description_html derivation, same-email no-reset, audit row, profile-image upload, subdomain-change redirect). Custom-domain host variant NOT testable under phpunit (L13 - domain-group routes never load); covered by P1 route snapshots + manual QA. |
| P15 | not started | | | | |
| P16 | tests-first done (observers pending) | merged to main @ 9ad67a11 | | 2026-07-11 | ModelBootClosureCharacterizationTest (9 pins): markdown *_html, starts_at re-key of sold JSON + sale event_date (venue-local via creatorRole->timezone; SaleTicket::created seeds sold), pass bucket never re-keyed, name/description translation resets (Event + Role), email lowercase + *_normalized recompute, hosted email-change verification reset + VerifyEmail notification, boost-campaign cancel on delete (status cancelled, meta_status null), orphaned unregistered role HARD-deleted with its only event. |
| P17 | tests-first partial (conversion pending) | merged to main @ 9ad67a11 | | 2026-07-11 | NewsletterValidationCharacterizationTest: store/update/storeSegment error-key pins + TWO ordering pins (store 404s on missing role_id BEFORE validation; update flashes newsletter_already_sent BEFORE validation, no errors bag) - both sites fail rule 5(a) gate 1 as written, so the guards must stay ahead of validation in any conversion. Remaining converted actions get pins during P17 itself. |
| P18 (opt) | tests-first partial | merged to main @ 9ad67a11 | | 2026-07-11 | manageMembers allowed (MiscFeaturesTest) + denied-403 (NewsletterValidationCharacterizationTest) pinned; remaining authorize() sites get pins during P18. |
| F1 | not started | | | | |
| F2a | not started | | | | |
| F2b | not started | | | | |
| F3 | not started | | | | |
...
| O2 (opt) | tests-first done (refactor pending) | merged to main @ 9ad67a11 | | 2026-07-11 | NoFakeEmailCharacterizationTest: blocked/allowed domains, example.com pre-check, substring-matching semantics pinned (do NOT "fix" to exact-domain), size pinned at 3632 (source-derived until the data file exists), message pinned. |
| O3 (opt) | not started | | | | |
