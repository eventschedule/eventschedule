...
| Event requests | Visitors submit events to a schedule through the AI Import form or the Booking Form (`Role::usesBookingForm()`; a talent always uses the Booking Form). Require Account, Require Approval, approved schedules and Request Terms are ungated, and so are the Booking Form's required default fields and its Online switch (`roles.booking_form_config`, `Role::bookingFormRequires()` / `bookingFormAllowsOnline()`). A talent's Booking Form never forces an account (`Role::bookingFormRequiresAccount()`), and the optional account box appears only where the install accepts new accounts (`public_registration_enabled()`). Asking your own questions on the form is the Pro custom fields feature |
