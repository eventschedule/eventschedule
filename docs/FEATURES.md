...
| Passes & subscriptions | `PassBookingService::isBookable()`; `EventRepo::saveEvent()` scrubs `is_pass` below Pro | Multi-use passes redeemable across events (visit pass, membership, festival pass, season pass); usage tracked on the Subscriptions tab; per-pass cancellation deadline and late-cancel policy (forfeit or block). The purchase confirmation falls back to the platform mailer like any ticket, but the advance-booking confirmation (`EmailService::sendPassBookingConfirmation()`) still needs the schedule's own email settings on hosted |
...
| Bulk attendee import | `TicketController::importAttendees`, `$event->isPro()` | Import attendees in bulk from CSV or form entry (up to 5,000 rows per import). Emailing them their tickets needs the schedule's own email settings (`TicketController::importAttendeesStore()` checks `hasEmailSettings()` with no hosted gate, so this holds on selfhost too), unlike checkout confirmations, which fall back to the platform mailer |
...
| WhatsApp event creation | **Missing.** Sold as Enterprise, but `WhatsAppWebhookController` has no plan check (a verified phone and an editor role are all it asks), so every plan can use it today. Add the gate, or re-label the feature | Create events via WhatsApp messages/images with AI parsing |
