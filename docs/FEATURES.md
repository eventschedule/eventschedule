...
| Subscribe to a schedule's calendar | `FeedController::icalFeed`, `/{subdomain}/feed/ical` (plus `/feed/rss`). A live feed the visitor's calendar re-reads, so a moved date updates itself - unlike the one-off `.ics` download below, which is a snapshot. Offered in the Add to Calendar menu on event and schedule pages. The route has always existed and was public and unauthenticated; until now it was surfaced only in a signed-in user's following list and the owner's own settings, so no guest could find it. Costs no email address and no consent, which is why it is the part of the interest feature that cannot fail on deliverability |
