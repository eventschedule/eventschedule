...
| Payment gateways (Stripe, Payfast, payment link, cash) | `config/payments.php` + `App\Services\Payments\PaymentGatewayManager`. Every gateway is available on every tier - there is NO `isPro()` check anywhere in `app/Services/Payments/`, and the connect route is gated on auth only. Connect one per account in Settings > Payment Methods and pick per event. **Payfast** settles in South African rand only, so it is offered only on ZAR events, and a Payfast event cannot join the multi-event cart or use installments. Invoice Ninja is the one gateway that IS Pro - see below |
