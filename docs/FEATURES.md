...
| Multiple team members | `RoleController:1210,1229`, `$role->isEnterprise()` | Add/manage multiple team members. An **admin** runs the schedule day to day and sees its ticket sales, waitlist and check-in dashboard (`User::manageableRoles()` / `Event::scopeManagedBy()`); a **viewer** is read-only and sees no sales, but may scan tickets at the door (`User::canScanEvent()`). Only the owner changes levels or removes members (`RolePolicy::manageMemberLevels()`) |
