...
- **ChromeDriver mismatch, the quiet version.** A **two**-major gap is the loud one above. A **one**-major gap is not: the session IS created, pages load, `script()` runs and Vue mounts, but ChromeDriver's synthesized input silently goes nowhere, so `click()`, `type()` and the Actions API become no-ops while anything driven through `script()` keeps working. On Chrome 153 against ChromeDriver 152 that cost `SeatingTest` 7 of its 10 journeys, and each one read as an app bug: a plan name that "never saved", a section that "did not move", seats that "were never held" - the 3 survivors were exactly the 3 that drive the screen with `dispatchEvent` instead of real presses. Chrome auto-updates and the vendored binary does not, so a dev machine drifts into this about every four weeks; CI does not, because it runs `dusk:chrome-driver --detect` every time. `DuskTestCase::guardAgainstChromeDriverDrift()` now compares the two majors in the `#[BeforeClass]` hook and aborts the class with the remedy rather than letting it surface as a wall of unrelated assertion failures.
