...
- **Never expose follower emails to schedule owners** - Schedule owners must not see their followers' email addresses. Use `name` instead of `email` in schedule-owner-facing views (stats, subscriber lists). Admin views may show emails.
