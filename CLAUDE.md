...
- **AP brand blue via CSS variables** - In AP/auth views, use CSS variables instead of hardcoded hex: `var(--brand-blue)` (primary), `var(--brand-blue-light)` (lighter), `var(--brand-blue-dark)` (hover/darker). These auto-adapt between light and dark mode. In Tailwind: `bg-[var(--brand-blue)]`, `hover:bg-[var(--brand-blue-dark)]`. In inline styles: `color: var(--brand-blue)`. In JS (e.g. Chart.js canvas): `getComputedStyle(document.documentElement).getPropertyValue('--brand-blue').trim()`. Do NOT add `dark:` variants for brand blue classes - the CSS variable handles dark mode automatically.
